...
# Insert sentence data into table
def insert_sentences(cursor):
    sentences = [
...
def create_db(cursor):
    try:
        # create database
        cursor.execute(
            f"CREATE DATABASE {DB_NAME} DEFAULT CHARACTER SET 'utf8' COLLATE utf8_general_ci;"
        )
        # create table
        ...
        for table_name in TABLES:
            ...
            try:
                print(f"Creating table {table_name}") # 확인용
...
try:
    ...
    insert_sentences(cursor)
    connection.commit() # 저장
    print("Sentences successfully inserted!")
except mysql.connector.Error as e:
